...
8. ROAS目標20向けの入札係数を一括適用する場合は `Boost Ops` → `0.3) ROAS20入札係数をconfigへ適用`
9. `input_bulk_sp_raw` だけで必要inputを自動作成する場合は `Boost Ops` → `0.4) RAWから必須inputを自動作成`
...
補足:
- `normal_campaign_name` に `Auto` / `オート` を含む行は、移行先候補として自動的に無視します。
...
- 移行入札は次で算出:
- `base_bid = input_mapping.default_bid（>0なら優先、無ければ Boost側CPC、さらに無ければ fallback_bid）`
- `final_bid = base_bid × ROAS帯係数（config）`
- 最後に `min_bid` / `max_bid` で上下限を適用

### 停止判定（Boost）
- `config.require_stop_approval=true` の場合:
  - `input_stop_products` で承認済みのASIN（`approved_to_stop=true`）は、Auto/ManualともCampaignをアーカイブ
  - `min_impressions_to_stop` は停止ゲートに使わない（露出判断はクライアント側）
- `config.require_stop_approval=false` の場合:
  - Campaign単位で`impressions`合計が`min_impressions_to_stop`以上ならアーカイブ
...
15. AmazonへアップロードするExcelだけ欲しい場合は `1.6) アップロード用Excelを作成（output_bulk_spのみ）`
...
- `pause_level`: 互換用の設定値。Boost停止出力は常にCampaignアーカイブ
