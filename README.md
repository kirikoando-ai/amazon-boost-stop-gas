...
9. `Boost Ops` → `0.4) RAWから必須inputを自動作成` を実行
10. `input_stop_products` で停止する `product_key` の `approved_to_stop=true` を設定（クライアント承認）
11. `Boost Ops` → `1) 判定＆出力作成`
12. `output_blockers` を確認し、必要なら `input_mapping` を修正
13. `Boost Ops` → `1) 判定＆出力作成` を再実行
