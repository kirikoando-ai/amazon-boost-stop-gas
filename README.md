...
8. `config_presets` を確認し、採用パターンの値を `config` に転記
9. `input_boost_campaigns` を更新（BoostキャンペーンIDと product_key を紐付け）
10. `Boost Ops` → `0.5) SP生データをinput_boostへ変換`
11. `input_mapping` を更新
12. `input_stop_products` にクライアント承認済みの停止対象商品を貼る（`approved_to_stop=true`）
13. `Boost Ops` → `1) 判定＆出力作成`
14. `output_blockers` を確認し、必要ならマッピング追加または `product_key` を補完
15. `Boost Ops` → `1) 判定＆出力作成` を再実行
16. `output_bulk_sp` をBulkアップロード（または `output_migrate_exact` と `output_pause_boost` を個別利用）
17. 必要なら `2) 出力シートをCSV化（Drive保存）`
